...
// Candidate applications endpoints - Store and retrieve job applications in MongoDB
app.post('/candidate-applications', async (req, res) => {
  try {
    const db = await connectMongoDB();
    if (!db) {
      return res.status(500).json({ success: false, error: 'Database connection failed' });
    ...
    const collection = db.collection('candidate_applications');
    ...
    const applicationData = {
      ...
      applications: [{
        ...
        company: req.body.company || 'HireZen',
    ...
    console.log('📝 Storing candidate application in MongoDB:', {
      email: applicationData.email,
      jobId: applicationData.applications[0].jobId,
      position: applicationData.applications[0].position
    });

    // Upsert - if candidate exists, add to applications array, otherwise create new
    const result = await collection.updateOne(
      { email: applicationData.email },
      {
        ...
        $push: {
          applications: applicationData.applications[0]
        },
        $setOnInsert: {
          createdAt: new Date()
        }
    ...
    console.log('✅ Candidate application stored in MongoDB:', result.upsertedId || 'updated existing');
    ...
    console.error('❌ Error storing candidate application:', error);
    res.status(500).json({ success: false, error: error.message });
  }
});

// Get candidate applications by email
app.get('/candidate-applications/:email', async (req, res) => {
  try {
    const db = await connectMongoDB();
    if (!db) {
      return res.status(500).json({ success: false, error: 'Database connection failed' });
    ...
    const candidate = await collection.findOne({ email: email });
    ...
    const applications = candidate.applications.sort((a, b) =>
